                                                    ...
                                                    G4Tubs* tubsSolid)
{
  ...
  // convert this to use the outer radius of the tube and check for the top/bottom boundaries.
  if(aStep->GetPreStepPoint()->GetStepStatus() == fGeomBoundary)
  {
    ...
    G4double OutsideRadius = tubsSolid->GetOuterRadius();
    if((std::fabs(localpos1.z() + tubsSolid->GetZHalfLength()) < kCarTolerance) || (std::fabs(localpos1.z() - tubsSolid->GetZHalfLength()) < kCarTolerance)) {
      return fCurrent_In;
    } else if(localR2 >
         (OutsideRadius - kCarTolerance) * (OutsideRadius - kCarTolerance) &&
       localR2 <
         (OutsideRadius + kCarTolerance) * (OutsideRadius + kCarTolerance))
    {
      return fCurrent_In;
  ...
  if(aStep->GetPostStepPoint()->GetStepStatus() == fGeomBoundary)
  {
    ...
    if((std::fabs(localpos2.z() + tubsSolid->GetZHalfLength()) < kCarTolerance) || (std::fabs(localpos2.z() - tubsSolid->GetZHalfLength()) < kCarTolerance)) {
      return fCurrent_Out;
    }
    if(localR2 >
         (OutsideRadius - kCarTolerance) * (OutsideRadius - kCarTolerance) &&
