...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  tableY = 182.88*cm;
  tableZ = 3.175*cm;
  tableCenter = G4ThreeVector(worldX*0.5 - sWallX - 71.12*cm - xOffset, worldY*0.5 - tableY*0.5 - pillarY - lowPillarY - 30.48*cm + yOffset, -worldZ*0.5 - tableZ*0.5 + floorZ + 76.2*cm + zOffset);
  // Construction
  ...
  G4ThreeVector sourceCapCenter = G4ThreeVector(PuBeCenterShield.x(), PuBeCenterShield.y(), PuBeCenterShield.z() + stainlessH*0.5 + capHeight*0.5 + 1.*um);
  G4cout << "PuBe source Coords:" << PuBeCenterShield.getX()/cm << ", " << PuBeCenterShield.getY()/cm << ", " << PuBeCenterShield.getZ()/cm << G4endl;
  // Construction:
  ...
  new G4PVPlacement(0, PuBeCenterShield, pubeCurrentLogic, "PuBeCurrentCyl", logicWorld, false, 0, checkOverlaps);
  ...
  if (isHe3) {
    ...
    G4LogicalVolume* topCapLogic = new G4LogicalVolume(topCapSolid, fmats["steel"], "TopCap");
    ...
    G4LogicalVolume* bottomCapLogic = new G4LogicalVolume(bottomCapSolid, fmats["steel"], "BottomCap");
